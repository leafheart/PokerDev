...
struct tagDataBaseInfo
{
	...
	TCHAR							szDataBaseAddr[128];				//数据库地址
...
struct tagGameServiceOption
{
	...
	TCHAR							szDataBaseAddr[128];				//数据地址
	TCHAR							szDataBaseName[SERVER_LEN];			//数据名字
	TCHAR							szGameRoomName[SERVER_LEN];			//房间名称
	TCHAR							szServerAddr[SERVER_LEN];			//房间地址
	
	TCHAR							szGameRoomRuleBak[RULE_LEN];			//房间规则
	TCHAR							szGameRoomRule[RULE_LEN];			//房间规则
	
	WORD							wPrivateType;						//自建场类型
	TCHAR							szPrivateRule[96];					//自建场规则
	WORD							wCreateType;
	TCHAR							szUserName[SERVER_LEN];				//房主名字
	WORD							wPlayerCount;						//是3人/4人游戏
	WORD							wGameJuShu;							//每张房卡打多少局
	WORD							wGameStartType;						//游戏开始方式；2：需要主动发送开始消息
	WORD							wDissmissWaitTime;					//申请解散时的等待时间，时间到后不选择同意/拒绝，自动解散
	BYTE							cbJoinInGameByCreate;				//创建房间选项：游戏开始后是否允许加入，在cbJoinInGame为true时才生效
