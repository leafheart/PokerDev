...
struct tag_DBR_Hongbao
{
	...
	DWORD							dwRoomNum;							//房间号
	WORD							wCount;								//数量
	TCHAR							szGameNum[32];						//房间唯一标示
	WORD							wActivityId;
	DWORD							dwContextID;
	WORD							iCode;								//领取失败0:成功；1：失败 
};	
//开房数据
struct tag_OtherFriendCreate
{
	DWORD							dwUserID;							//用户ID
	DWORD							dwUserPlayerID[4];							//用户ID1
	int								dwWinScore[4];
	TCHAR							cUserName[4][32];						//用户名1
	TCHAR							szGameNum[32];						//牌局
	DWORD							dwRoomNum;							//房间号
	WORD							wKaPrice;
	WORD							wKindID;
	WORD							wType;								// 未开始:0  开始:1  解散退卡:3  结束:2  解散退卡:4  异常解散 5
	WORD							wCount;
	WORD							wPlayers;			//人数
	TCHAR							szRule[512];						//规则
...
//开房数据返回
struct tag_OtherFriendCreateReq
{
	DWORD							dwUserID;							//用户ID
	TCHAR							szUserName[32];						//用户名
};
...
struct tagServerUserData
{
	...
	TCHAR								szGroupName[GROUP_LEN];				//社团名字。TODO:暂时复用这个字段来保存ip信息
	TCHAR								szUnderWrite[UNDER_WRITE_LEN];		//渠道号？
	TCHAR								szFaceUrl[256];						//自定义头像
	TCHAR								szDescribeString[128];				//个性签名
	...
	BYTE								cbUserStatusBeforeOffline;			//离线前的用户状态，用来重连后恢复状态
	...
	// 房间桌子列表位置信息
	unsigned short						m_usBeginTableIndex;				// 玩家AOI起始桌子ID
	unsigned short						m_usEndTableIndex;					// 玩家AOI结束桌子ID
	bool								m_bAtTableList;						// 玩家在桌子列表界面标志
...
public:
	...
	virtual BYTE __cdecl GetUserStatusBeforeOffline()=NULL;
	//用户胜率
...
public:
	...
	//GPS 数据
	virtual double __cdecl GetLongitude()=NULL;
	//GPS 数据
	virtual double __cdecl GetLatitude()=NULL;
	virtual int __cdecl GetgpsType()=NULL;
	virtual void __cdecl SetLocation(int _type,double _longdate, double _latDate)=NULL;
	
	virtual char* __cdecl GetAddr()=NULL;
	virtual void __cdecl SetAddr(char szAddr[], WORD nLen)=NULL;
...
public:
	//发送用户状态数据
	virtual bool __cdecl SendUserStatus(WORD wChairID,  BYTE cbStatus)=NULL;
	//发送数据
	...
	//发送所有玩家（包括自己）是否同意解散房间的状态
	virtual bool __cdecl SendAllPlayerDissmissInfo(IServerUserItem * pIServerUserItem)=NULL;
	//发送所有玩家（包括自己）的解散状态
	virtual bool __cdecl sendUserReadyState(IServerUserItem * pIServerUserItem,bool states[],int n)=NULL;
...
public:
	//单局游戏写入成绩
	virtual void __cdecl TableWriteGameDate(WORD wPlayNum, WORD wChairID, enScoreKind ScoreKind,  LONG lWinScore)=NULL;
	//总局游戏写入成绩
	virtual void __cdecl TableWriteGameDateTotal(WORD wPlayCount, WORD wChairID, enScoreKind ScoreKind,  LONG lWinScore)=NULL;
	//牌局记录
	virtual void __cdecl TableWriteOutDate(WORD wPlayNum, LPCTSTR pszHandCard, LPCTSTR pszOutCard)=NULL;
	//扣房卡
	virtual void __cdecl TableDeleteFangKa()=NULL;
	//扣房卡，每一局扣一张房卡
	virtual void __cdecl TableDeleteFangKa(int curCompleteCount)=NULL;
	//扣房卡
	virtual void __cdecl TableDeleteDaYingJiaFangKa(DWORD nDaYingJiaID)=NULL;

	virtual void __cdecl TableHongbaoSet(WORD playCount)=NULL;

	virtual void __cdecl TableAddCreateOtherFriend(tag_OtherFriendCreate *createInfo)=NULL;
};
...
public:
	...
	//发送状态
	virtual bool __cdecl SendkickingUserStatus(IServerUserItem * pIServerUserItem,DWORD userid)=NULL;
	virtual bool __cdecl SendUserStatusV2(IServerUserItem * pIServerUserItem,  BYTE cbStatus)=NULL;
	//发送道具
	virtual bool __cdecl SendResidualProperty(IServerUserItem * pIServerUserItem)=NULL;

	virtual bool __cdecl SendUserStartGame(IServerUserItem * pIServerUserItem)=NULL;
...
public:
	//游戏记录
	virtual void __cdecl WriteGameDateTotal(LPCTSTR pszGameNum, DWORD dwTableNum, WORD wPlayCount, WORD wChairID, DWORD dwUserID, enScoreKind ScoreKind, LONG lWinScore)=NULL;
	//游戏记录
	virtual void __cdecl WriteGameDate(LPCTSTR pszGameNum, DWORD dwTableNum, WORD wPlayNum, WORD wChairID, DWORD dwUserID, enScoreKind ScoreKind, LONG lWinScore)=NULL;
	//出牌记录
	virtual void __cdecl WriteOutDate(LPCTSTR pszGameNum, DWORD dwTableNum, WORD wPlayNum, LPCTSTR pszHandCard, LPCTSTR pszOutCard)=NULL;
	//扣房卡
	virtual void __cdecl DeleteFangKa(IServerUserItem * pIServerUserItem, LPCTSTR pszGameNum, DWORD dwTableNum, WORD nCount, WORD nPlayers, LPCTSTR pszRule)=NULL;
	//扣大赢家房卡
	virtual void __cdecl DeleteDaYingJiaFangKa(DWORD dayingjiaUserID, LPCTSTR pszGameNum, DWORD dwTableNum, WORD nCount, WORD nPlayers, LPCTSTR pszRule)=NULL;
	//加房主房卡
	virtual void __cdecl  AddFangZhuFangKa(IServerUserItem * pIServerUserItem, LPCTSTR pszGameNum, DWORD dwTableNum, WORD nCount, WORD nPlayers, LPCTSTR pszRule)=NULL;
	virtual void _cdecl AddCreateOtherFriend(tag_OtherFriendCreate *createInfo) = NULL;//DWORD wUserID, LPCTSTR pszGameNum, DWORD dwTableNum, WORD kindId, WORD _type, LONG _time, WORD nCount, LPCTSTR pszRule)= NULL;
	
	virtual void _cdecl GetHongBao(tag_DBR_Hongbao *hbInfo)=NULL;

	virtual void _cdecl AddCreateVipFriend(tag_OtherFriendCreate *createInfo)=NULL;
// 坐桌
public:
	// 广播指定桌子数据变更
	virtual bool _cdecl BroadCastTableUpdate(WORD sTableID, CMD_GR_TableListUpdate& rstTableListUpdate)=NULL;
	// 判断是否是坐桌模式
	virtual bool _cdecl IsSit()=NULL;
};
