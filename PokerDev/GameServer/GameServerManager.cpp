...
CGameServiceManager::CGameServiceManager(void)
{
	...
	lstrcpyn(m_GameServiceAttrib.szDescription,TEXT("肉挨肉游戏服务组件"),CountArray(m_GameServiceAttrib.szDescription));
	lstrcpyn(m_GameServiceAttrib.szClientModuleName,TEXT("RouAiRouServer.EXE"),CountArray(m_GameServiceAttrib.szClientModuleName));
	lstrcpyn(m_GameServiceAttrib.szServerModuleName,TEXT("RouAiRouServer.DLL"),CountArray(m_GameServiceAttrib.szServerModuleName));
...
void * __cdecl CGameServiceManager::CreateTableFrameSink(const IID & Guid, DWORD dwQueryVer)
{
	...
	try
	{
		...
		pTableFrameSink=new nsRouAiRou::CGameLogic();
...
bool __cdecl CGameServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption)
{
	...
	nsRouAiRou::g_strRoomRule = pGameServiceOption->szGameRoomRule;
